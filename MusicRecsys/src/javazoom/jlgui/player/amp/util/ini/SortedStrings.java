...
public class SortedStrings extends Alphabetizer implements Cloneable {
  ...
  public void add( String num ) {
    if( string_count == 0 || greaterThan( num, strings[string_count-1] ) ) {
      if( string_count == strings.length )
        strings = (String[]) Array.grow( strings, growth_rate );
  ...
  public void remove( String num ) {
    ...
    if( index < string_count && equalTo( strings[index],num ) )
      removeIndex( index );
  }
  ...
  protected int search( String num ) {
    ...
    while( true ) {
      if( lb >= ub - 1 ) {
        if( lb < string_count && !greaterThan( num, strings[lb] ) )
          return lb;
        else
          return lb + 1;
      }
      ...
      else if( lessThan( num, index_key ) )
        ub = index;
