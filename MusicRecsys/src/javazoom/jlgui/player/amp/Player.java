...
public class Player extends Window implements ActionListener, BasicPlayerListener, DropTargetListener, WindowListener
{
  ...
  protected static String TITLETEXT = "jlGui 2.3 ";
  ...
  public void setTracker (Tracker tFrame)
  {
      this.tFrame = tFrame;
  ...
  public void setCurrentSong(PlaylistItem pli)
  {
    ...
    {
        /// @@@@ R2ROKID9 touched this: Lets make playlists repeat all the time
      // Try to repeat ?
  ...
  public void stateUpdated(BasicPlayerEvent event)
  {
	log.debug("Event:"+event);
        System.out.println("Currently: "+playerState);
	...
	if (state==BasicPlayerEvent.EOM)
	{
	  if ((playerState == PAUSE) || (playerState == PLAY))
	  {
              // @R2ROKID9: The repeat for playlists will be for the particular music instead. :D
                if (!config.isRepeatEnabled())
                {   
		  playlist.nextCursor();
		  fileList.nextCursor();
